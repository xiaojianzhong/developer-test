...
/**
 * This class is used to parse command-line arguments based on a the variables
 * with an object. Each variable in the class specifies a command-line argument
 * that can be accepted; the name of the variable becomes the name of the
 * command-line argument. As Java does not allow dashes '-' in variables, use
 * two underscores '__' if you want a dash in an argument. Also, if you want to
 * have a numeric argument (e.g.: '-1'), start the variable name with a single
 * underscore. JCLO uses reflection to determine the type of each variable in
 * the passed object and sets the values in the object passed to it via parsing
 * the command line. If you include a String array named "additional", all
 * non-dashed arguments will be placed in it.
 *
 * @author Steve Beaty (beatys@mscd.edu) @version $Id: JCLO.java,v 1.5
 *         2007/11/01 16:43:12 beaty Exp beaty $
 */
...
public class JCLO {
  ...
  /**
   * A constructor that takes the Object that contains the variables acceptable on
   * a command line. Call parse (String) to do the actual parsing.
   *
  ...
  /**
   * A constructor that takes an Object, a prefix, and a boolean that specifies
   * whether to accept single or double dashes; call parse (String) to do the
   * actual parsing.
   *
   * @param object  where the variables/arguments are
  ...
  public String usage() {
    ...
    for (Field field : fields) {
      ...
      list.add((doubleDashes ? "--" : "-") + key + getUsageType(type) + "\n");
    }
  ...
  /**
   * Add to object o to the end of the array contained in field and return the
   * resulting array.
   *
  ...
  /**
   * Make an Object of the correct type for the field, using a String version of
